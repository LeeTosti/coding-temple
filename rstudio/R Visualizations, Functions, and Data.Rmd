...
```{r}
arrange(filter(flights, month==3, day==31))
```
...
#select all columns fromt he flights dataset that start with the letter c:
...
#select all columns that match the genx expression fl|c:
...
#apply a mutation to to the dataset:
...
#apply a transmutation to to the dataset:
#will only return new columns
...
```{r}
view(select(cars, starts_with('m')))
```

```{r}
view(select(cars, ends_with('g')))
```

```{r}
view(select(cars, matches('mp|c')))
```

```{r}
cars_mut <- mutate(cars, disp_by_cyl = disp * cyl)
view(cars_mut)
```

```{r}
my_var <- group_by(cars, mpg, cyl, gear)

view(summarize(my_var))
...
```{r}
avg_speed_mut <- mutate(flights, avg_speed =distance / (air_time / 60))

my_pipe <- avg_speed_mut %>%
  filter(carrier !='UA', !is.na(avg_speed)) %>%
  group_by(sched_arr_time) %>%
  summarize(carrier, avg_speed, origin, dest)
...
ggplot(myplot, aes(class, hwy)) +
  ...
  labs(title = 'Highway MPG by Car Class', c = 'Car Class', y= 'Highways MPG') +
  theme_bw() +
  coord_flip()
```
...
```{r}
ggplot(Batting,aes(yearID, HR)) +
  geom_col(yearID, HR, color = 'HR', fill= 'HR', binwidth=20)
...
calc_mean <- function(x){
  ...
  if(length(x) == 0){
    ...
    return(paste('Mean of our vecotr is:', sum/length(x)))
  }
}
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    find_smallest_int(c(34, 15, 88, 2))
...
```{r}
#lat, long, depth, and mag are all continuous
#stations are discrete
```

```{r}
filter(quakes, mag > 5.0)
```

```{r}
arrange(quakes, stations)
```
...
#apply a mutation to to the dataset:
quake_data_mut <- mutate(quake_data,
                          location = paste(lat, '_', long),
                          intensity = 11.8 + 1.5*mag)
view(quake_data_mut)
```

```{r}
summarize(quake_data_mut, mean_intensity = mean(intensity, na.rm=TRUE))
```

```{r}
ggplot(data=quake_data_mut, mapping = aes(mag, intensity, color = mag < 5)) +
  geom_point()
```
