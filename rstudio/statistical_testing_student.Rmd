...
```{r}
df %>% summarise(na_count = sum(is.na(df))) #preferred
# same as python df.isna().sum().sum()
...
```{r}
df%>%summarize(data_type = class(df))
...
```{r}
df_red <- my_fun(df_red)
df_red
```

```{r}
df_red %>% summarise(na_count = sum(is.na(df_red))) #preferred
# same as python df.isna().sum().sum()
...
# X increases (noisily)
# Z increases slowly
# Y is constructed so it is inversely related to xvar and positively related to x var * z var
set.seed(99)
